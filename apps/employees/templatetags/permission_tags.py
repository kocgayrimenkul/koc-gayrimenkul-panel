...
@register.filter
def can_view_calls(user):
    """Çağrılar sayfasını görüntüleme izni - Yönetici, Santral, Santral/Sekreter"""
    if not user.is_authenticated:
        return False
    if user.is_superuser:
        return True
...
@register.filter
def is_danishman(user):
    """Kullanıcının Danışman rolünde olup olmadığını kontrol eder"""
    if not user.is_authenticated:
        return False
    if user.is_superuser:
        return False
    return user.groups.filter(name='Danışman').exists() and not user.groups.filter(
        name__in=['Yönetici', 'Santral', 'Santral/Sekreter']).exists()
